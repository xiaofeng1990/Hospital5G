...
class XfConfig {
  ...
  private:
    GKeyFile *_config_key_file = nullptr;
    std::string _config_file_path;
    bool _save_file = false;
...
bool XfConfig::Get(const std::string group, const std::string key, T &value)
{
    ...
    gvalue = g_key_file_get_value(_config_key_file, group.c_str(), key.c_str(),
                                  &error);
    if (error)
    {
...
bool XfConfig::Set(const std::string group, const std::string key, T value)
{
    ...
    os << value;
    g_key_file_set_value(_config_key_file, group.c_str(), key.c_str(),
                         os.str().c_str());
    if (!_save_file)
        _save_file = true;
